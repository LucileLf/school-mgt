...
import { ClassSchema, SubjectSchema, classSchema, subjectSchema } from "@/lib/formValidationSchemas";
import { createSubject, updateSubject, createClass, updateClass } from "@/lib/actions";
  ...
  return (
    <form className="flex flex-col gap-8 " onSubmit={onSubmit}>
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Create a new class" : `Update ${data?.name}`}
      </h1>
      {/* {`Update ${data?.name}`} */}
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select
            className="ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full"
            {...register("gradeId")}
            defaultValue={data?.grades?.map((grade: {id: string}) => grade.id)}
          >
            {grades.map(
